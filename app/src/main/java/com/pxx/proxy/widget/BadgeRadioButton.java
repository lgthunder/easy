...
public class BadgeRadioButton extends DrawableCenterRadioButton {
    ...
    /**
     * 画圆文本长度，没有字符串或者字符串长度为1（数字0-9），画圆
     */
    private static final int DRAW_CIRCLE_TEXT_LENGTH =1;
    ...
    private void drawWithNoDrawable(Canvas canvas) {
        ...
        if (mBadgeText.length() <= DRAW_CIRCLE_TEXT_LENGTH) {
            //绘制圆
    ...
    void init() {
        ...
        mBadgeTextPaint.setTypeface(Typeface.DEFAULT);
