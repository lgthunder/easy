...
class BackgroundTaskManager private constructor(private val app: Application) {
    ...
    private fun processQueue() {
        executor.execute {
            while (true) {
                ...
                try {
                    executeTrim(processing, cancelFlag, processing.includeAudio)
                    if (cancelFlag.get()) {
    ...
    private fun executeTrim(task: BackgroundTask, cancelFlag: AtomicBoolean, includeAudio: Boolean = false) {
        ...
        android.util.Log.d("VideoTrim", "Include audio: $includeAudio")
        ...
        try {
            ...
            val firstSamplePerTrack = LongArray(trackCount) { -1L }
            ...
            for (i in 0 until trackCount) {
                ...
                } else if (mime.startsWith("audio/") && includeAudio) {
                    muxerTrackIndices[i] = muxer.addTrack(format)
                    extractor.selectTrack(i)
                    android.util.Log.d("VideoTrim", "Track $i: $mime (audio)")
                }
            }

            if (muxerTrackIndices.all { it < 0 }) {
                throw RuntimeException("No video tracks found")
            }
            ...
            while (!cancelFlag.get()) {
                ...
                if (muxerTrackIndices[trackIndex] >= 0) {
                    ...
                    if (sampleSize >= 0) {
                        val pts: Long
                        if (firstSamplePerTrack[trackIndex] < 0) {
                            firstSamplePerTrack[trackIndex] = sampleTime
                            pts = 0L
                        } else {
                            pts = sampleTime - firstSamplePerTrack[trackIndex]
                        }
                        bufferInfo.apply {
                            offset = 0
                            size = sampleSize
